...
const SingleLeagueCard = ({ league, reload }) => {
    ...
    const join = async () => {
        if (me) {
            try {
                const { status } = await leagueServices.joinLeague(
                    league.leagueID
                );
    ...
    const enter = () => {
        ...
        if (me) context.openLeaguePage(league.leagueID);
        else {
            Attention("Please sign in first.");
            context.goTo(Routes.Client.SignUp);
        }
        ...
        >
            ...
            {showMore && (
                <Card.Footer>
                    {!ready ? (
                        <Row>
                            {league.contesters instanceof Array &&
                                !league.contesters.map(c => c.userID).includes(me.userID) && (
                                    <Col>
                                        <Button
                                            block
