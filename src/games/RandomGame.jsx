...
const RandomGame = () => {
    ...
    const onStartGameClick = (event) => {
        ...
        // random game:
            ...
            Attention("Please sign in first.");
    ...
    useEffect(() => {
        if (searching) {
            if (room.name) {
                OK(
                    "A random opponent has been found. The game will start once both players confirm."
                );
                setSearching(false);
            } else if (!randomSearchRepeats) {
                //means no one has been found after a specific amount of time
                Notify("No opponent found. Please try again shortly.");
                setSearching(false);
            }
        }
        ...
        <Fragment>
            ...
            <Form onSubmit={(event) => onStartGameClick(event)}>
                <Form.Group className="form-inline">
                    <Form.Label className="pb-2 w-25">Game Type</Form.Label>
                    <InputGroup
                        style={{
                    ...
                    >
                        <InputGroup.Prepend className="mx-3">
                            ...
                            />
                            <InputGroup.Text>Scored</InputGroup.Text>
                        </InputGroup.Prepend>
                        <InputGroup.Prepend className="mx-3">
                            <InputGroup.Radio
                                value="1"
                ...
                <Form.Group className="form-inline">
                    <Form.Label className="pb-2 w-25">Board Size</Form.Label>
                    <InputGroup
                        style={{
                            border: "1px solid orange",
                            borderRadius: "5px",
